...
import { previewGroups, previewModulesByGroup } from '@/lib/demo/demoCurriculum';
...
export default function GroupPage({ params }: { params: Promise<{ chapter: string; group: string }> }) {
  const { chapter, group } = use(params);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const fallbackGroups = previewGroups.filter(g => (mode === 'both' ? true : g.teaching_mode === mode));
      const resolvedGroups = liveGroups.length ? liveGroups : fallbackGroups;

      const activeGroup = resolvedGroups.find((g: Group) => g.code === group);
      setCurrentGroup(activeGroup || null);

      if (!activeGroup) {
        setModules(((previewModulesByGroup[group] || []).filter(m => (mode === 'both' ? true : m.teaching_mode === mode))) as ModuleRow[]);
        return;
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/teacher/curriculum/${chapter}`} className="text-sm text-gray-700 hover:text-gray-900">← Back to Lesson Groups</Link>
      </div>
          ...
          {sortedModules.map((mod, idx) => {
            ...
            return (
              <div key={mod.id} className={clsx('rounded-2xl border p-4 shadow-sm flex items-center justify-between',
                ...
                <Link
                  href={locked ? '#' : `/teacher/curriculum/${chapter}/${group}/${mod.code}`}
                  className={clsx(
