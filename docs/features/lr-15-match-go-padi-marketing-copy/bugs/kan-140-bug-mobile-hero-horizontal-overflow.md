...
created: 2026-05-11
updated: 2026-05-11
fixed_in: LR-15-uat-2
---

## Fix verified (2026-05-11, LR-15-uat-2)

Eng_fix attempt 1 landed cleanly. The `app/page.tsx` changes:
- Eyebrow chip is now responsive — `<span className="md:hidden">Free early access for ages 3–7</span>` on mobile, `<span className="hidden md:inline">Now in free early access for teachers of 3- to 7-year-olds</span>` on desktop.
- Subtitle `max-w-2xl` → `max-w-full md:max-w-2xl`.
- Both hero grid cells got `min-w-0`.

CDP measurement at 375×667 now returns `document.body.scrollWidth === clientWidth === 375` and `hasHorizontalScroll: false`. All 11 LR-15 ACs pass round-2 UAT.

See `docs/features/lr-15-match-go-padi-marketing-copy/uat/LR-15-uat.md` for the full round-2 verdict.
